...
class MilkQualityFilter(django_filters.FilterSet):
    date = django_filters.DateFilter(lookup_expr='exact', label='Data (AAAA-MM-DD)')
    animal__name = django_filters.CharFilter(lookup_expr='icontains', label='Nome do Animal')
    ...
    class Meta:
        model = MilkQuality
        fields = ['date', 'animal__name', 'producer']
